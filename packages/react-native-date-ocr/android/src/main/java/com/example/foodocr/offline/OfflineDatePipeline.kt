...
class OfflineDatePipeline private constructor(
    ...
    companion object {
        ...
        /**
         * Create the pipeline from filesystem paths instead of bundled assets.
         * Used by the React Native bridge where models are downloaded to filesDir
         * (e.g. /data/user/0/<app>/cache/ocr/ocr.bundle/yolo_date.onnx).
         */
        fun createFromFiles(
            ...
            val detector = YoloDateDetector(
                env = env,
                modelBytes = java.io.File(yoloModelPath).readBytes(),
                inputSize = yoloInputSize,
            )
            val recognizer = PpocrDateRecognizer(
                env = env,
                modelBytes = java.io.File(recModelPath).readBytes(),
                keys = java.io.File(recKeysPath).readLines(Charsets.UTF_8),
                runAllVariants = runAllRecVariants,
            )
            return OfflineDatePipeline(env, detector, recognizer)
        }
